...
let ProductCompra = prompt(
  ...
  let totalCuotas=0;
  let TipoPago = parseInt(prompt(`${nom_User}, indica que metodo de pago deseas usar:
  1. Debito
  2. Credito
  3. Efectivo`)
);
...
let cuotas = parseInt( prompt(`${nom_User}, indica que cuota deseas usar (maximo 48 cuotas):`));
if (cuotas > 0 && cuotas <= 48 ) {
totalCuotas += total / cuotas;
alert(`${nom_User}, el total a pagar es de ${totalCuotas} pesos en ${cuotas} cuotas. muchas gracias por su compra. `);
break;
} else {
cuotas = parseInt(
prompt(`${cuotas} no es uina opcion valida, indica que cuota deseas usar:`)
);
}
...
TipoPago = parseInt(
prompt(`${nombreCliente}, indica que metodo de pago deseas usar:
  1. Debito
  2. Credito
  3. "Efectivo"`)
);
